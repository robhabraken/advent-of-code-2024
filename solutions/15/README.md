...
### Update: moving boxes faster!
As mentioned, I already realized when finishing day 15 this wasn't the quickest approach (it was the easiest to build though). The reason for this is mostly because I stored the walls and boxes in a `List<Obstacle>` and I had to iterate over this list to find the boxes and walls around me for each move and each box to move subsequently. But the boxes and walls I am looking for are always right next to me, so there's no reason to search through all obstacles - other than the fact that that is just rather easy to build.

So, after finishing all AoC puzzles I came back to this one (among others I'm revisiting) and decided to change the collection type to a two-dimnensional `Obstacle[,]` array. This takes a bit more work, because I now need to actually build up the map and place the objects in there, and also, when moving boxes around, I have to actually move the obstacle within the array as well, instead of only updating its x and y attributes. And I also had to change over the if-statements, selecting the obstacles around me. But those, despite being longer in terms of LoC, are actually way easier to read. And that's also where the profit is: I now can just directly look in the cells around me within the array. If there's an obstacle, I can immediately retrieve it. And I now only have to interact with the objects I'm actually moving or bumping into.

Other than that, the whole concept and algorithm actaully stayed the same. But now, instead of 117 ms, it runs in 1.37 ms!